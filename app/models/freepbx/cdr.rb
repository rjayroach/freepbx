module Freepbx
  class Cdr < ActiveRecord::Base
    ...
    db_suffix = (%w(development test).include? Rails.env) ? "_#{Rails.env}" : ''
    ...
    ransacker :calldate do
      Arel::Nodes::SqlLiteral.new("date(#{table_name}.calldate)")
    end
    ...
    def self.group_by_src_and_hour
      group_by_hour 'src, u.extension,', 'u.extension,'
    end
    ...
    def self.group_by_hour(fields = '', group = '')
      select("#{fields} hour(calldate) as hour, sum(disposition = 'ANSWERED') as completed, sum(CASE WHEN disposition='ANSWERED' THEN duration ELSE 0 END) as total_duration, count(*) as total_calls").group("#{group} hour")
    end
    ...
    def self.ransack_params_for(day)
      ...
      { 'c' => { '0' => { 'a' => { '0' => { 'name' => 'calldate' } }, 'p' => 'gteq', 'v' => { '0' => { 'value' => day.to_s } } }, '1' => { 'a' => { '0' => { 'name' => 'calldate' } }, 'p' => 'lteq', 'v' => { '0' => { 'value' => day.to_s } } }, '2' => { 'a' => { '0' => { 'name' => '' } }, 'p' => 'eq', 'v' => { '0' => { 'value' => '' } } } } }
    end
    ...
    def self.placed_today
      placed_on Time.now.to_s.split[0]
    end

    def self.placed_yesterday
      placed_on Time.now.yesterday.to_s.split[0]
    end
    ...
    def self.placed_on(begin_date)
      placed_between begin_date, begin_date
    end
