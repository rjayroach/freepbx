...
module FreePbx
  class ElastixUser < ActiveRecord::Base
    establish_connection 'elastix_acl' if %w(development production).include? Rails.env
    ...
    def password_match?(pwd)
      md5_password.eql? Digest::MD5.hexdigest pwd
    end

    def self.sync_to_asterisk_users
      AsteriskUser.all.each do |ast_user|
        next if find_by_extension ast_user.extension
